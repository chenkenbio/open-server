...
.icon-button svg { display: block; width: 1.1em; height: 1.1em; }
</style>
</head>
<body>
...
{{if .HasParent}}<tr><td><a href="{{.ParentURL}}">Parent Directory</a></td><td>&nbsp;</td><td align="right"> - </td><td align="right">&nbsp;&nbsp;<button type="button" class="copy-path" data-path="{{.ParentPath}}">Path</button></td><td align="right">&nbsp;</td>{{if .TensorBoardEnabled}}<td align="right">{{if .ParentTensorBoard}}<form action="{{.ParentTensorBoard}}" method="post" target="_blank" style="margin: 0;"><input type="hidden" name="csrf" value="{{.CSRFToken}}"><button type="submit">Launch</button></form>{{else}}&nbsp;{{end}}</td>{{end}}{{if .LaTeXEnabled}}<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>{{end}}</tr>
{{end}}{{range .Entries}}<tr><td><a href="{{.URL}}">{{.Name}}{{if .IsDir}}/{{end}}</a>{{if .IsLink}}&nbsp;→ {{.LinkTarget}}{{if .Broken}} (broken){{end}}{{end}}</td><td>&nbsp;&nbsp;{{time .ModTime}}&nbsp;&nbsp;</td><td align="right">{{if .IsDir}} - {{else}}{{size .Size}}{{end}}</td><td align="right">&nbsp;&nbsp;<button type="button" class="copy-path" data-path="{{.FullPath}}">Path</button></td><td align="right">{{if .IsDir}}&nbsp;{{else}}&nbsp;&nbsp;<button type="button" class="download-file icon-button" data-href="{{.Download}}" aria-label="Download {{.Name}}" title="Download {{.Name}}"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v10m0 0 4-4m-4 4-4-4M5 17v3h14v-3"></path></svg></button>{{end}}</td>{{if $view.TensorBoardEnabled}}<td align="right">{{if .TensorBoard}}<form action="{{.TensorBoard}}" method="post" target="_blank" style="margin: 0;"><input type="hidden" name="csrf" value="{{$view.CSRFToken}}"><button type="submit">Launch</button></form>{{else}}&nbsp;{{end}}</td>{{end}}{{if $view.LaTeXEnabled}}<td align="right">{{if .TableSnippet}}<button type="button" class="copy-snippet" aria-label="Copy LaTeX table snippet" title="Copy LaTeX table snippet" data-snippet="{{.TableSnippet}}">Table</button>{{else}}&nbsp;{{end}}</td><td align="right">{{if .FigureSnippet}}<button type="button" class="copy-snippet" aria-label="Copy LaTeX figure snippet" title="Copy LaTeX figure snippet" data-snippet="{{.FigureSnippet}}">Figure</button>{{else}}&nbsp;{{end}}</td><td align="right">{{if .LiveURL}}<button type="button" class="open-live" aria-label="Open live PDF preview in a new tab" title="Open live PDF preview in a new tab" data-href="{{.LiveURL}}">Preview</button>{{else}}&nbsp;{{end}}</td>{{end}}</tr>
